...
def move_robot_to_rz(mc, rz_angle):
    ...
    try:
        # --- 1. 변수 정의 및 최종 Rz 계산 ---
        # Rz 각도 추론값 (rz_angle)을 사용하여 최종 목표 Rz를 계산
        # (사용자 코드에서 +90을 한 것으로 보아, 로봇 좌표계에 맞춘 Rz 최종값이라고 가정)
        rz_final = round(float(rz_angle) + 90, 2)

        # 최종 잡기 목표 좌표 (X, Y, Z, Rx, Ry, Rz_final)
        # PICK_COORDS는 [X, Y, Z_PICK, Rx, Ry, Rz_DEFAULT] 형태라고 가정합니다.
        PICK_COORDS_FINAL = PICK_COORDS[:5] + [rz_final]
        # PICK_COORDS_FINAL[2]는 최종 잡기 높이(Z_PICK)입니다.
        Z_PICK = PICK_COORDS_FINAL[2] 

        # --- 2. 접근 높이(Z_APPROACH) 정의 ---

        # 최종 잡기 높이(Z_PICK)보다 안전하게 높은 접근 높이 정의 (예: 50mm 위에 설정)
        Z_APPROACH_OFFSET = 50.0 # 안전 접근 높이 오프셋 (mm 단위)
        Z_APPROACH = Z_PICK + Z_APPROACH_OFFSET 

        # --- 3. 2단계 동작 실행 ---

        # **[1단계] 접근 자세로 이동 (High Z)**
        # Rz를 포함한 모든 자세를 정확히 잡되, 높이만 높은 Z_APPROACH로 설정
        approach_coords = PICK_COORDS_FINAL[:] # 최종 좌표 복사
        approach_coords[2] = Z_APPROACH       # Z 값만 접근 높이로 변경

        print(f"1. Approach Coords (High Z): {approach_coords}")
        # mc.send_coords(approach_coords, speed=50)
        mc.send_angles(INTERMEDIATE_POSE_ANGLES, speed=50)
        time.sleep(8) # 자세를 잡을 충분한 시간 부여 (50mm 내려가는 시간보다 길어야 함)

        # **[2단계] 최종 잡기 자세로 수직 하강 (Low Z)**
        # Z_APPROACH에서 Z_PICK으로 수직 하강.
        # Z 값 외에는 변경이 없으므로 로봇은 수직 운동만 수행합니다.
        target_coords = PICK_COORDS_FINAL 

        print(f"2. Target Coords (Low Z): {target_coords}")
        mc.send_coords(target_coords, speed=20) # 수직 하강은 안전을 위해 속도를 낮춥니다.
        time.sleep(2)
