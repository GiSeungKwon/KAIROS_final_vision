...
data_transforms = {
    'train': transforms.Compose([
        ...
        transforms.RandomResizedCrop(224, scale=(0.8, 1.0)), # 크롭 및 리사이즈
        transforms.RandomHorizontalFlip(), # Rz 값이 반전되는 문제가 발생할 수 있으므로, 실제 로봇 환경에서 확인 필요
...
def train_model():
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        if avg_val_loss < best_val_loss:
            ...
            model_path = os.path.join(BASE_DIR, 'best_multitask_model.pth')
            torch.save(model.state_dict(), model_path)
            print(f"  -> Model saved: New best validation loss {best_val_loss:.4f}")

        # B. 10 에포크마다 모델 저장
        if epoch % SAVE_INTERVAL == 0:
            model_path_interval = os.path.join(BASE_DIR, f'multitask_model_epoch_{epoch}.pth')
