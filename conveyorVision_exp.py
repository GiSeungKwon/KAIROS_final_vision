...
import numpy as np
import os
import math # <-- [추가] 지수 함수 사용을 위해 math 라이브러리 추가
...
# [추가] 지수 함수 스케일링을 위한 상수 (AD loss의 차이를 증폭시키는 역할)
LOSS_SCALING_FACTOR = 20 

# ⚠️ 임계값 설정 (각 보드별 통계 분석 후 설정된 값 사용)
# [수정] 지수 함수 변환(e^(loss * LOSS_SCALING_FACTOR))에 맞춰 임계값도 재설정해야 합니다.
# (예시: 0.045 -> e^(0.045 * 20) ≈ 2.46)
AD_THRESHOLDS = {
    "ESP32": 2.5,  
    "L298N": 2.5, 
    "MB102": 3.4,
            ...
            with torch.no_grad():
                ...
                if ad_detector:
                    ...
                    if roi.size > 0:
                        ...
                        original_ad_loss = torch.mean((ad_input_tensor - ad_output_tensor) ** 2).item()
                        
                        # -----------------------------------------------------------
                        # 💡 [수정] 지수 함수를 적용하여 Loss 차이를 극대화
                        # -----------------------------------------------------------
                        scaled_ad_loss = math.exp(original_ad_loss * LOSS_SCALING_FACTOR) # e^(loss * alpha)
                        
                        threshold = AD_THRESHOLDS[predicted_class]
                        
                        # 결과 판별 (scaled_ad_loss 사용)
                        if scaled_ad_loss > threshold:
                            ad_status = "ANOMALY"
                        ...
                        inspection_result["ad_loss"] = scaled_ad_loss # Scaled loss를 저장
